...
function PatronView({ venue, menu, BRAND }) {
  ...
  const drinkReady = orderStatus === "ready";
    ...
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ padding: "0 20px 100px" }}>
          ...
          {cart.length === 0 ? (
            ...
            <>
              ...
              {isDemoMode && (
                <div style={{ padding: "12px 16px", background: BRAND.accent + "15", border: `1px solid ${BRAND.accent}33`, borderRadius: 10, marginBottom: 16, textAlign: "center" }}>
                  <span style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: BRAND.accent, letterSpacing: 2 }}>DEMO MODE — PAYMENT SIMULATED</span>
                </div>
              )}
              ...
              <button onClick={handleCheckout} style={{ width: "100%", padding: "18px", background: `linear-gradient(135deg, ${BRAND.primary}, ${BRAND.accent})`, border: "none", borderRadius: 14, color: BRAND.white, fontFamily: "'Oswald', sans-serif", fontSize: 18, fontWeight: 700, letterSpacing: 3, cursor: "pointer", boxShadow: `0 4px 30px ${BRAND.primaryGlow}` }}>
                {isDemoMode ? "PLACE ORDER" : `PAY $${(totalCents / 100).toFixed(2)}`}
              </button>
      ...
      {view === "confirmation" && order && (
        <div style={{ padding: "30px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 28, minHeight: "60vh", justifyContent: "center" }}>
          <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: drinkReady ? 20 : 13, fontWeight: drinkReady ? 700 : 600, letterSpacing: 4, color: BRAND.success, textTransform: "uppercase", textAlign: "center", transition: "all 0.5s ease" }}>
            {drinkReady ? "YOUR DRINK IS READY" : "Order Confirmed"}
          </div>
          ...
          <ConfirmationBadge
            letter={order.confirm_letter}
            color={{ hex: order.confirm_hex, name: order.confirm_color }}
            timestamp={new Date(order.ordered_at).getTime()}
            drinkReady={drinkReady}
          />
          ...
          <button onClick={() => { setOrder(null); setOrderStatus(null); setView("menu"); }} style={{ marginTop: 12, padding: "14px 32px", background: "transparent", border: `1px solid ${BRAND.dimText}`, borderRadius: 12, color: BRAND.gray, fontFamily: "'Oswald', sans-serif", fontSize: 14, fontWeight: 500, letterSpacing: 2, cursor: "pointer" }}>
            NEW ORDER
          </button>
        </div>
      )}
